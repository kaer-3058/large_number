...
#Z≈(X-N^2)/(2N+1)
#X越大，估值越精确，因此改进版估值算法，是在X小于九位数的时候不断计算X*100直到X位数到达八位或九位
...
execute store result score #1.0 int store result score #1.1 int store result score #1.2 int store result score #1.3 int run scoreboard players operation output.sqrt int = #inp int
...
execute store result score #7.1 int store result score #6.5 int store result score #6.3 int store result score #6.2 int run scoreboard players get output.sqrt int
