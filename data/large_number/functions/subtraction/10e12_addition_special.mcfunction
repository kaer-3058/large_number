...
#输入第一个数：data modify storage math addition.input1 set value [I;0,0,0]
#输入第二个数：data modify storage math addition.input2 set value [I;0,0,0]
#输入规范：必须是三个万进制的整数，若数位为空则写0

#输出：data get storage math addition.output
#或：data get storage math addition
#读数规范：输出的列表每个数都是万进制数，若数位不足四位则向前补0

#相加
execute store result score #4.4 int run scoreboard players operation #1.4 int += #2.4 int
...
execute store result score #4.3 int run scoreboard players operation #1.3 int += #4.4 int
...
scoreboard players operation #1.4 int %= 10000 const
...
execute if score #+- int matches 1 unless score #4.1 int matches ..-1 unless score #1.1 int matches ..-1 unless score #1.2 int matches ..-1 unless score #1.3 int matches ..-1 if score #1.4 int matches 1.. run scoreboard players operation #1.4 int *= -1 const

#输出
data modify storage math subtraction.output set value [I;0,0,0,0,0]
execute store result storage math subtraction.output[0] int 1.0 run scoreboard players get #4.1 int
execute store result storage math subtraction.output[1] int 1.0 run scoreboard players get #1.1 int
execute store result storage math subtraction.output[2] int 1.0 run scoreboard players get #1.2 int
execute store result storage math subtraction.output[3] int 1.0 run scoreboard players get #1.3 int
execute store result storage math subtraction.output[4] int 1.0 run scoreboard players get #1.4 int
